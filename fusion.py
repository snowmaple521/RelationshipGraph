...
class BAN(nn.Module):
    ...
    def forward(self, v_relation, q_emb, b):
        ...
        b_emb = [0] * self.glimpse #【40，1024】
        # b x g x v x q
        # att：torch.Size([40, 1, 36, 14])
        # att_logits：torch.Size([40, 1, 36, 14])
        ...
        for g in range(self.glimpse):
            ...
            #如果使用计数模型，传入位置信息和注意力特征
            if self.use_counter:
                embed = self.counter(boxes, atten) #boxes:torch.Size([40, 4, 36])atten:torch.Size([40, 36])
                q_emb = q_emb + self.c_prj[g](embed).unsqueeze(1) #q_emb:torch.Size([40, 14, 1024])
